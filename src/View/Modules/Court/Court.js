...
const Court = ({ match }) => {
  ...
  const handleCastVote = async (voteDetails) => {
    ...
    const { accountId, milestoneId, votedFor, customerRating, workerRating } =
      voteDetails;
    await acceptJuryDutyAndCastVoteTx(
      api,
      accountId,
      milestoneId,
      votedFor,
  ...
  const handleSudoJurorCastVote = async ({
    accountId,
    milestoneId,
    votedFor,
    customerRating,
  ...
  return (
    <>
      ...
      />
      <CourtDetails milestone={milestone} />
      <br />
      <PotentialJurors
      ...
      Object.keys(milestone.dispute.finalJurors).length !== 0 ? (
        <>
          <FinalJurors dispute={milestone.dispute} />
          <br />
        </>
      ) : null}
      ...
      <CourtSummary dispute={milestone.dispute} />
      ...
      <VoteModal
        ...
        handleClose={() => {
          setShowVoteModal({ show: false, onClickHandler: null });
        }}
      />
    </>
  );
