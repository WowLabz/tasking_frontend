...
import { transferUsingPalletBalances } from "../../../palletTaskingFunctions";
...
const CardForAirDrop = () => {
  ...
  const handleFormSubmit = async (data) => {
    try {
      let AliceFromKeyRing = keyring.getAccount(DEFAULT_ACCOUNT_IDS.ALICE);
      ...
      >
        ...
        >
          <div>
            ...
            <h4 style={{ margin: "0" }}>
              You can request air drop to get 500 tokens for testing
            </h4>
          </div>
          <Button
            variant="success"
                    ...
                    <Card>
                      ...
                      <Card.Body>
                        <FormLabelAndDropDown
                          ...
                          onChange={(e) => {
                            setFieldValue("accountName", e.target.value, false);
                            let val;
                            [...connectedAccounts]?.forEach((acc) => {
                              if (acc.meta.name === e.target.value) {
                                val = acc.address;
                                return;
                              }
                            });
                      ...
                      <Card.Footer className="d-flex justify-content-center aligin-items-center">
                        <Button variant="dark" type="submit" name={`Air Drop`}>
                          <b>{`Air Drop`}</b>
                        </Button>
                      </Card.Footer>
                    </Card>
