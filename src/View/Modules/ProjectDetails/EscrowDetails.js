                ...
                <div>
                    ...
                    <List bulleted>
                        ...
                        {milestone.status !== MILESTONE_STATUS.Open && (
                            <List.Item>
                                Bid Accepted <i>added</i> {milestoneCost} Units from {milestone.workerName}
                            </List.Item>
                        )}
                        {milestone.status === MILESTONE_STATUS.Completed && !milestone.dispute && (
                            <List.Item>
                                Milestone Completed <i>transferred</i> {2 * milestoneCost} Units to {milestone.workerName}
                            </List.Item>
                        )}
                        {milestone.status === MILESTONE_STATUS.Completed && milestone.dispute !== null && (
                            Object.keys(milestone.dispute.finalJurors).map(juror => {
                                let courtFees = milestoneCost * 0.6 * 2;
                                const jurorCount = getFinalJurorcount(milestone.dispute.finalJurors);
                                return(<List.Item>
                                    Juror Fees <i>transferred</i> {courtFees/jurorCount} Units to {getJuror(juror)}
                                </List.Item>);
                            })  
                        )}
                        {milestone.status === MILESTONE_STATUS.Completed && milestone.dispute !== null && (
                            <List.Item>
                                Court Adjourned <i>transferred</i> {milestoneCost*0.4*2} Units to {milestone.dispute.winner === 'Worker' ? milestone.workerName : publisher }
                            </List.Item>
                        )}
                    </List>
                </div>
