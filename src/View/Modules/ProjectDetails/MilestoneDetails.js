...
const MilestoneDetails = (props) => {
  ...
  const [showMilestoneCompletedModal, setShowMilestoneCompletedModal] =
    useState(false);
  ...
  useEffect(() => {
    ...
    getListOfBidders();
  }, [milestone, api, api?.query.palletTasking]);

  const attributes = getAttributesForMilestone(milestone);

  const onBidForMilestone = async () => {
    await bidForMilestoneTx(
      api,
      props.walletUser.address,
      milestone.milestoneId,
      props.walletUser.name
    );
  };
  ...
  const handleProvideCustomerRating = async (ratingForCustomer) => {
    await provideCustomerRatingsTx(
      api,
      props.walletUser.address,
      milestone.milestoneId,
      ratingForCustomer
  ...
  return (
    <div>
      ...
      <Accordion.Content active={activeIndex === index + 1}>
        <Segment>
          ...
          <List bulleted>
            ...
            <List.Item>Milestone Deadline: {milestone.deadline}</List.Item>
            <List.Item>
              Publisher Attachment Link:{" "}
              <a href={milestone.publisherAttachments} target="_blank">
                IPFS Link
              </a>
            </List.Item>
            ...
            props.walletUser.address === project.publisher ? (
              <List.Item>
            ...
            {milestone.workerId !== null ? (
              <List.Item>Worker Id: {milestone.workerId}</List.Item>
            ) : null}
            ...
            {milestone.workerAttachments !== null ? (
              <List.Item>
                Worker Attachments Link:{" "}
                {milestone.workerAttachments ? (
                  <a href={milestone.workerAttachments} target="_blank">
                    {" "}
                    IPFS Link
                  </a>
                ) : (
          ...
          <div>
            ...
            {milestone.workerName !== null &&
            milestone.status === MILESTONE_STATUS.InProgress &&
            props.walletUser.address === milestone.workerId ? (
              <div>
                <Button
                  basic
                  floated="right"
            ...
            {milestone.status === MILESTONE_STATUS.PendingApproval &&
            props.walletUser.address === project.publisher ? (
              <div>
            ...
            {project.status === "Open" &&
            milestone.status === MILESTONE_STATUS.CustomerRatingPending &&
            props.walletUser.address === milestone.workerId ? (
              <div>
                <Button.Group floated="right">
                  <div style={{ padding: "5px" }}>
            ...
            {project.status === "Open" &&
            milestone.status === MILESTONE_STATUS.CustomerRatingProvided &&
            props.walletUser.address === project.publisher ? (
              <div>
                <Button.Group floated="right">
                  <div style={{ padding: "5px" }}>
      ...
      <ConfirmModal
        ...
        handleClose={() => {
          setShowConfirmModal({ show: false, onClickHandler: null });
        }}
